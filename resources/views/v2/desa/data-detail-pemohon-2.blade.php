...
@section('title',"Data Pemohon $data->nama")
@section('breadcrumb')
...
<div class="alert alert-danger">
    <p style="margin:0" class="text-center">{{session('gagal')}}</p>
</div>
@elseif(session('ubah'))
<div class="alert alert-warning">
    <p style="margin:0" class="text-center">{{session('ubah')}}</p>
</div>
@endif
<div class="row">
    ...
    <div class="col-lg-4 col-xlg-3 col-md-5">
        <div class="card">
            <div class="card-body">
                ...
                <h6>Jalan {{$data->jalan}}, RT: {{$data->rt}} RW: {{$data->rw}} <br> {{$data->jenis_daerah}}
                    {{$data->nama_daerah}} Kecamatan Pemalang</h6>
                <small class="text-muted">Diajukan pada</small>
                <h6>{{$hari.date('d',strtotime($tanggal))}} {{$m}} {{date('Y',strtotime($tanggal))}} <br> Pukul:
                    {{date('H:i:s',strtotime($tanggal))}}</h6>
            </div>
        </div>
    </div>
